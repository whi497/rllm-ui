...
export const LogsPanel: React.FC<LogsPanelProps> = ({ logs, isLoading }) => {
  ...
  return (
    <div className="h-full flex flex-col bg-layer-1">
      {/* Toolbar */}
        ...
        {matchIndices.length > 0 && (
          <div className="flex items-center gap-1">
            <button
              ...
              className="p-1.5 hover:bg-layer-2 rounded text-gray-500"
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium rounded-md transition-colors ${
            followMode
              ...
              : "bg-layer-2 text-gray-500 hover:text-gray-700"
          }`}
        >
      ...
      >
        {isLoading ? (
          <div className="flex items-center justify-center h-full">
            <Spinner size="sm" variant="blue" label="Loading logs..." />
          </div>
        ) : logs.length === 0 ? (
          <EmptyState
            icon={
...
const LogLine = React.memo(
  ...
  >(({ log, formatTime, activeQuery, isCurrentMatch }, ref) => {
    return (
      <div
        ref={ref}
        className={`flex gap-2 px-1 hover:bg-layer-1 text-gray-700 ${
